...
# === حساب المالك لاستلام الأموال (Placeholder) ===
...
def wallet_connect(request: Request):
    ...
    return request.app.templates.TemplateResponse(
        "wallet_connect.html",
        {"request": request, "title": "إعداد حساب الاستلام", "session_user": u}
    )

@router.post("/wallet/connect")
def wallet_connect_post(request: Request):
    # لاحقاً: توليد رابط Stripe Account Link ووضعه في Redirect
    return RedirectResponse(url="/wallet/settings", status_code=303)
...
def checkout_deposit(item_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    # لاحقاً: سنقرأ security_deposit من الجدول، الآن نعرض رقم تجريبي
...
def checkout_deposit_post(item_id: int, request: Request):
    # لاحقاً: إنشاء جلسة دفع Stripe / تفويض تأمين
