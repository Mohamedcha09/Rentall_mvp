...
# ---------------------------
# Auto-close after 24h (queue=mod)
# ---------------------------
...
def mod_assign_self(ticket_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    name = (request.session["user"].get("first_name") or "").strip() or "مدقّق المحتوى"
    try:
...
def mod_ticket_reply(tid: int, request: Request, db: Session = Depends(get_db), body: str = Form("")):
    ...
    now = datetime.utcnow()
    db.add(SupportMessage(
        ticket_id=t.id, sender_id=u_mod["id"], sender_role="agent",
    ...
    try:
        name = (request.session["user"].get("first_name") or "").strip() or "مدقّق المحتوى"
        push_notification(db, t.user_id, "💬 رد من فريق المراجعة (MOD)",
...
def mod_resolve(ticket_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    db.add(SupportMessage(
        ticket_id=t.id, sender_id=u_mod["id"], sender_role="agent",
        body=f"تم إغلاق التذكرة بواسطة {name} (MOD) في {now.strftime('%Y-%m-%d %H:%M')}",
        created_at=now
    ))

    t.unread_for_user = True
    try:
        push_notification(db, t.user_id, "✅ تم حل تذكرتك (MOD)",
...
def mod_transfer_to_md(ticket_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    now = datetime.utcnow()
    # انقل للطابور MD + بصمة تحويل system
    t.queue           = "md"
    ...
    db.add(SupportMessage(
        ticket_id=t.id, sender_id=u_mod["id"], sender_role="system",
        body="🔁 تم تحويل التذكرة إلى إدارة الودائع (MD).", created_at=now
    ))
    ...
    try:
        push_notification(db, t.user_id, "🔁 تم تحويل تذكرتك",
                          f"تذكرتك #{t.id} تحوّلت إلى إدارة الودائع (MD).",
                          url=f"/support/ticket/{t.id}", kind="support")
        db.commit()
    except Exception:
        db.rollback()
    ...
    try:
        ...
        for (md_id,) in md_users:
            push_notification(db, md_id, "📩 تذكرة جديدة من MOD",
                              f"توجد تذكرة محوّلة من فريق المراجعة (MOD): #{t.id}",
                              url=f"/md/ticket/{t.id}", kind="support")
        db.commit()
    except Exception:
        db.rollback()
