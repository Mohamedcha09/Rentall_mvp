...
def booking_flow_page(
    booking_id: int, request: Request, db: Session = Depends(get_db),
    user: Optional[User] = Depends(get_current_user),
):
    require_auth(user)
...
def owner_decision(
    ...
    if decision == "rejected":
        ...
        db.commit(); return redirect_to_flow(bk.id)
...
def renter_choose_payment(
    ...
    bk.payment_method = "online"; bk.timeline_payment_method_chosen_at = datetime.utcnow()
    db.commit(); return redirect_to_flow(bk.id)
...
def _legacy_owner_confirm_return_admin(
    ...
    if bk.status not in ("returned", "in_review", "picked_up"):
        return redirect_to_flow(bk.id)
    if bk.status != "in_review": bk.status = "in_review"
    ...
    if dep == 0:
        ...
        if action == "ok":
            ...
            if amt >= dep: bk.deposit_status = "claimed"; bk.deposit_charged_amount = dep
            else: bk.deposit_status = "partially_withheld"; bk.deposit_charged_amount = amt

    bk.owner_return_note = (owner_note or "").strip()
    bk.status = "closed"; bk.return_confirmed_by_owner_at = now; bk.timeline_closed_at = now
    db.commit(); return redirect_to_flow(bk.id)

# ---------------- (إدمن) قرار الديبو النهائي ----------------
